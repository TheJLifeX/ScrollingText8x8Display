/*
 font source: https://github.com/dhepper/font8x8/blob/master/font8x8_basic.h, http://www.gammon.com.au/forum/?id=11516
 Each font value index corresponds to ascii decimal value of the corresponding char. see: https://www.cs.cmu.edu/~pattis/15-1XX/common/handouts/ascii.html
 For example 'A' has a ascii decimal value of 65 and it is stored at font_8x8[65].

 We cant initialize the full font_8x8 array in the SRAM. It is too big. So we use `PROGMEM`.
 The `PROGMEM` arduino keyword tells the compiler to store data in flash (program) memory instead of SRAM. readmore: https://www.arduino.cc/reference/en/language/variables/utilities/progmem/
*/
...
const byte font_8x8[NUMBER_OF_CHARACTERS][8] PROGMEM = {
